...
# def to_sql(BookRentals,BookRentals_exception,customers,customers_exception):
...
#     BookRentals.to_sql('BookRentals', con=engine, if_exists='append', index=False)
#     BookRentals_exception.to_sql('BookRentals_exception', con=engine, if_exists='append', index=False)
#     customers.to_sql('customers', con=engine, if_exists='append', index=False)
#     customers_exception.to_sql('customers_exception', con=engine, if_exists='append', index=False)
    
    
def main():
   # Add the data cleaning steps
    

    #load the csv's as dataframes
    systembook = open_file('./Data_raw/03_Library Systembook.csv')
    ...
    BookRentals.to_csv('./Data_cleansed/BookRentals.csv', index =False)
    BookRentals_exception.to_csv('./Data_cleansed/BookRentals_exception.csv', index =False)
    
    ##customers 
    #load the csv as a dataframe
    customers = open_file('./Data_raw/03_Library SystemCustomers.csv')
    ...
    #saving df as csv
    customers.to_csv('./Data_cleansed/customers.csv', index =False)
    customers_exception.to_csv('./Data_cleansed/customers_exception.csv', index =False)
    
    print(customers.head())
    #to_sql(Books,BookRentals,BookRentals_exception,customers,customers_exception)
